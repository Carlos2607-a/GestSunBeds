...
Sunbed sales tracker with:
  - A welcome/login screen (logo + username/password)
  - Two roles: "seller" (Register tab only) and "admin" (Register + History)
...
# ============================================================
# USERS -- CHANGE THESE before going live. Passwords are stored
# hashed (sha256), never in plain text, but do not commit real
# passwords to a public repo -- keep this repo private, or move
# these to environment variables later.
# ============================================================
...
def login_page():
    with ui.column().classes("w-full items-center justify-center gap-4").style("min-height: 90vh;"):
        # Logo placeholder -- replace with ui.image('logo.png') once you have
        # a real logo file saved next to app.py.
        ui.icon("beach_access", size="64px").classes("text-blue-400")
        ui.label("Sunbed Sales").classes("text-3xl font-medium")
        ui.label("Beach club sunbed sales tracker").classes("text-gray-500 mb-4")
...
def main_app_page():
    ...
    if role == "admin":
        ...
        # Sellers only see the register screen, no tabs needed
        register_content()
